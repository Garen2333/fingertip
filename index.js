...
function downloadData() {
    ...
    csvContent += "Time,Glucose,Lactate,Vitamin C\n";

    let maxLength = Math.max(glucoseData.length, lactateData.length, vitaminData.length);

    for (let i = 0; i < maxLength; i++) {
        let glucoseValue = glucoseData[i] ? glucoseData[i].value : "";
        let lactateValue = lactateData[i] ? lactateData[i].value : "";
        let vitaminValue = vitaminData[i] ? vitaminData[i].value : "";
        let time = glucoseData[i] ? glucoseData[i].time : "";

        csvContent += `${time},${glucoseValue},${lactateValue},${vitaminValue}\n`;
    }
